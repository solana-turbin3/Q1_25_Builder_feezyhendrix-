...
mod tests {
    ...
    fn transfer_sol() {
        let keypair =
            read_keypair_file("./src/dev-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("QNUfNMaB8dEWGrW3pWw7YEoSg6akdfdcRqV6y4zNSvz").unwrap();
    ...
    fn enroll() {
        let rpc_client = RpcClient::new(RPC_URL);
        let signer =
            read_keypair_file("./src/turbin3-wallet.json").expect("Couldn't find wallet file");
